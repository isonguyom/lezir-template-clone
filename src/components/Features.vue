    ...
    <div class="container">
      <div class="card text-center mb-5 border-0 bg-transparent mx-auto col-lg-6">
        <div class="card-body">
          ...
          <p class="card-text opacity-75">Et harum quidem rerum facilis est et expedita distinctio nam libero tempore cum
            soluta nobis eligendi cumque.</p>
        </div>
      </div>
        ...
        <div class="d-lg-flex justify-content-between align-items-center flex-wrap feature mb-5"
          ...
          <div class="col-lg-5 py-5">
            <p class="card-title mb-4">
              ...
              </svg> CREATIVE FEATURES
            </p>
            <h4 class="card-title mb-4">{{ feature.title }}</h4>
            <p class="card-text opacity-75">{{ feature.details }}</p>
            <p class="card-text opacity-75" v-for="item, index in feature.meta" :key="index"><span class="pe-1 opacity-75"
                v-html="item.icon"></span> {{ item.text }}</p>
  ...
  data() {
    return {
      features: [
        ...
        {
          title: "We do the work you stay focused on your customers.",
          details: "Temporibus autem quibusdam et aut officiis debitis aut rerum a necessitatibus saepe eveniet ut et voluptates repudiandae sint molestiae non recusandae itaque.",
          meta: [
